...
def download_app_data(file_name: str) -> str:
  ...
  cache_dir = file_utils.convert_to_posix_path(
      file_utils.get_local_tmp_directory(), "scendroid", "app_data"
  )
  remote_url = (
      f"https://storage.googleapis.com/gresearch/scendroid/{file_name}"
  )
...
class ScenDroidApp(AppSetup):
  """Class for setting up Android World app.

  ScenDroid app provides on-screen visualization of tasks and rewards.
  """
