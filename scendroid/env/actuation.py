...
def execute_adb_action(
  ...
  if action.action_type in ['click', 'double_tap', 'long_press']:
  ...
  elif action.action_type == 'delete_text':
    adb_utils.delete_text(env)
    ...
    if text:
      if action.index is not None or (
        ...
        time.sleep(1.0)
      adb_utils.delete_text(env)
      adb_utils.type_text(text, env, timeout_sec=12)
    ...
    if direction == 'down':
      ...
      end_x, end_y = 0, mid_y
    elif action.start_x is not None and action.start_y is not None: # Custom swipe
...
def find_and_click_element(
    element_text: str,
    env: scendroid_controller.ScenDroidController,
