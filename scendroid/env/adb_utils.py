...
def delete_text(
    ...
    """Deletes all text by moving cursor to end and long pressing delete key."""
    # Move the cursor to the end
    logging.info('Moving cursor to the end')
    move_response = press_keyboard_generic(
        keycode="KEYCODE_MOVE_END",
        env=env,
        timeout_sec=timeout_sec,
    )
    if move_response.status != adb_pb2.AdbResponse.Status.OK:
        logging.error('Failed to move cursor to the end')
        return move_response

    # Long-press the Delete key to clear the text
    logging.info('Long pressing delete key')
    del_response = issue_generic_request(
        args=['shell', 'input', 'keyevent', '--longpress', 'KEYCODE_DEL'],
        env=env,
        timeout_sec=timeout_sec,
    )
    if del_response.status != adb_pb2.AdbResponse.Status.OK:
        logging.error('Failed to long press delete key')
